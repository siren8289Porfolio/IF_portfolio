...
이 폴더는 고령자 일자리/지역 데이터를 이용해서 **로그인 없이도 바로 쓸 수 있는 PoC → 모델/설명 레이어 → API 서비스**까지 한 번에 가는 실험용 프로젝트입니다.
...
elder-risk-ai/
  ...
  data/
    raw/           # 원본 CSV/XLSX (수정 금지, 여기로 복사해 두기)
...
> 현재 `data/raw`에는 사용자가 직접 CSV/XLSX 파일을 복사해 두어야 합니다.  
> 예: `ai/` 폴더에 있는 파일들을 `elder-risk-ai/data/raw`로 옮기면 됩니다.
...
1. `data/raw`에 아래 파일들을 넣습니다.
   ...
   - `국민건강보험공단_노인장기요양보험 등급판정 현황_20251231.CSV` (아직 점수에는 안 씀)
2. 인코딩 변환 + parquet 저장:
